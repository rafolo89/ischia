...
//funzione che ritorna il layer contenente il tragitto
        ...
        .factory('myPoiss', function ($http, promiseFactory, $localStorage) {
            return{
                myPois: function () {
                    ...
                    var queryMyPois = prefixQuery
                            ...
                            + "FILTER(regex(str(?subject),'http://sws.geonames.org/" + $localStorage.uid + "/')).}";
                    ...
                    $http.get(queryUrl)
                            .success(function (data, status, headers, config) {
                                if (data.results.bindings.length != 0) {
                                    ...
                                    window.myJson[3] = null;
                                }
                                return dfd.resolve("ciao");
                            })
                            .error(function (status)
                            {
                                var alert = $ionicPopup.alert({
